...
public class CreditTest extends TestCase {
	 Credit test = null;
	    ...
	    protected void setUp() throws Exception {
			super.setUp();
	        this.accountTest = new Account(465,LocalDateTime.now(),50000, false,1, new InterestType4());
	    ...
	    public void testInterest2() {
			this.test = new Credit(LocalDateTime.now(), accountTest, 100000, 0, new InterestType2());
			assertEquals(this.test.GetAmount(),(this.test.GetBalance() * 0.02 + this.test.GetBalance()));
	    }

	    public void testInterest3() {
			this.test = new Credit(LocalDateTime.now(), accountTest, 100000, 0, new InterestType3());
			assertEquals(this.test.GetAmount(),(this.test.GetBalance() * 0.02 + this.test.GetBalance()));
		}

	    public void testInterest4() {
	    	this.test = new Credit(LocalDateTime.now(), accountTest, 100000,0,new InterestType4());
